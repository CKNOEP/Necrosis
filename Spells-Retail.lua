...
--]]
	...
	[2362]	= {UsageRank = 1, SpellRank = 1, Timer = false, Usage = "spellstone", Length = 60, Cooldown = 180,},
	[17727]	= {UsageRank = 2, SpellRank = 2, Timer = false, Usage = "spellstone", Length = 60, Cooldown = 180,},
	[17728]	= {UsageRank = 3, SpellRank = 3, Timer = false, Usage = "spellstone", Length = 60, Cooldown = 180,},
	...
	[6366]  = {UsageRank = 1, SpellRank = 1, Timer = false, Usage = "firestone",},
	[17951] = {UsageRank = 2, SpellRank = 2, Timer = false, Usage = "firestone",},
	[17952] = {UsageRank = 3, SpellRank = 3, Timer = false, Usage = "firestone",},
	[17953] = {UsageRank = 4, SpellRank = 4, Timer = false, Usage = "firestone",},
	...
	--[[ Demonic Sacrifice (Broken in MOP)
...
-- buff Demonic Sacrifice
	...
	[86121] = {UsageRank = 1, SpellRank = 1, Timer = false, Usage = "soul_swap"}, -- Soul Swap || Échange d'âme
	...
	--]]
	-- Soulstone usage results (Cataclysm only - don't exist as castables in MOP)
	-- In MOP there is only one Soulstone spell (20707) for creation - commented out
	...
	-- These Result spells don't exist as castables in MOP - commented out
	--[[ Healthstone usage results (Vanilla/TBC only)
...
function Necrosis.IsSpellKnown(usage)
	...
	then
		...
		-- First check InSpellBook flag (catches most spells)
		if Necrosis.Warlock_Spells[spellID] and Necrosis.Warlock_Spells[spellID].InSpellBook then
			-- For optional talents, also check IsSpellKnownOrOverridesKnown to ensure they're selected
			-- If InSpellBook is true but the spell is optional and not selected, this will return false
			if IsSpellKnownOrOverridesKnown(spellID) then
				return true
			else
				-- InSpellBook true but spell not known = optional talent not selected
				return false
			end
		end
		return false
...
function Necrosis.IsSpellRez(name)
	...
	--print (name)
	if name == (Necrosis.Warlock_Spell_Use["soulstone"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["soulstone"]].Name or "") -- MOP: Soulstone
	or name == (Necrosis.Warlock_Spell_Use["minor_ss_used"] and Necrosis.Warlock_Spells[Necrosis.Warlock_Spell_Use["minor_ss_used"]].Name or "")
...
--    print("SpellSetup")
	...
	local function Update(id, sub_name, spell)
		...
		--	if self.Warlock_Spells[id].Usage == "soulstone" then
			...
			.." '"..id.."'"
			.." N:'"..tostring(self.Warlock_Spells[id].Name or "nyl").."'"
...
--		Necrosis.Warlock_Spells[id].Length = 0 -- For timers, although the API appears to return values once cast
		...
		if Necrosis.Warlock_Spells[id].Result then
			...
			if Necrosis.Warlock_Spells[id].Name then
