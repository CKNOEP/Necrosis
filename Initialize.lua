...
-- CRITICAL FIX: Create placeholder button immediately!
-- This placeholder will be destroyed and recreated properly after initialization
local nbutton = CreateFrame("Button", "NecrosisButtonPlaceholder", UIParent, "SecureUnitButtonTemplate")
-- Register it as NecrosisButton for the rest of the code (will be replaced later)
...
function Necrosis:Initialize(Config)
	...
	if NecrosisConfig.NecrosisColor then
	else
	NecrosisConfig.NecrosisColor = "Rose" -- Default sphere color
	end
	...
	if not InCombatLockdown() then
		...
		C_Timer.After(3, function()
			if not InCombatLockdown() then
				...
				-- Create BRAND NEW button with historical name for position saving
				local btn = CreateFrame("Button", "NecrosisButton", UIParent, "SecureUnitButtonTemplate")
				btn:SetNormalTexture("Interface\\AddOns\\Necrosis\\UI\\Shard")
				btn:SetHighlightTexture("Interface\\AddOns\\Necrosis\\UI\\Shard", "ADD")  -- Glowing effect on hover
				btn:SetWidth(58)
				btn:SetHeight(58)
				...
				-- Create threat ring around the sphere
