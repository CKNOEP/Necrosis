...
Necrosis.Data = {
	AppName = "Necrosis",
	Version = "8.8.2",  -- Update manually when releasing
	Enabled = false,
}
...
C_Timer.After(1, function()
	...
	if Necrosis and Necrosis.HealthstoneUpdateAttribute then
		...
		Necrosis:SoulstoneUpdateAttribute()  -- Initialize soulstone (was missing!)
	end
end)

-- BUGFIX: Re-initialize stone attributes after BagExplore has found items (3 second delay)
-- This ensures ItemSwitchCombat table is properly populated before buttons are configured
C_Timer.After(3, function()
	if Necrosis and Necrosis.StoneAttribute then
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[Initialize] Re-initializing stone attributes after bag scan")
		Necrosis:StoneAttribute(true) -- Reinitialize all stone button attributes
	end
end)
