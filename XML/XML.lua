...
--	frame:SetScript("OnEnter", function(self) Necrosis:BuildTooltip(self, stone, "ANCHOR_LEFT") end)
	...
	-- Update stone attributes on click || Mettre à jour les attributs de la pierre au clic
	frame:SetScript("PreClick", function(self, button)
		-- Debug: Show click detection
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[PreClick] " .. b.f .. " clicked! button = " .. tostring(button))

		-- Update stone attributes before click || Mettre à jour les attributs de la pierre avant le clic
		if stone.f == Necrosis.Warlock_Buttons.health_stone.f then
			_G["DEFAULT_CHAT_FRAME"]:AddMessage("[PreClick] ✅ Healthstone detected")
			Necrosis:HealthstoneUpdateAttribute(false)
		elseif stone.f == Necrosis.Warlock_Buttons.spell_stone.f then
			_G["DEFAULT_CHAT_FRAME"]:AddMessage("[PreClick] ✅ Spellstone detected")
			Necrosis:SpellstoneUpdateAttribute(false)
		elseif stone.f == Necrosis.Warlock_Buttons.fire_stone.f then
			_G["DEFAULT_CHAT_FRAME"]:AddMessage("[PreClick] ✅ Firestone detected")
			Necrosis:FirestoneUpdateAttribute(false)
		elseif stone.f == Necrosis.Warlock_Buttons.soul_stone.f then
			_G["DEFAULT_CHAT_FRAME"]:AddMessage("[PreClick] ✅ Soulstone detected")
			Necrosis:SoulstoneUpdateAttribute(false)
			if not (InCombatLockdown() or UnitIsFriend("player","target")) then
				self:SetAttribute("unit", "player")
			end
		else
			_G["DEFAULT_CHAT_FRAME"]:AddMessage("[PreClick] ❌ Stone not recognized! stone.f = " .. tostring(stone.f))
		end
	end)

	-- PostClick for soulstone: reset unit back to target || PostClick pour le soulstone: réinitialiser l'unité
