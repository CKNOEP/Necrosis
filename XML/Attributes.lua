...
local function SetSSAttribs(nostone, reason)
	...
	f = _G[f]
	_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] Called! f=" .. tostring(f) .. " nostone=" .. tostring(nostone) .. " reason=" .. tostring(reason))
	if not f then
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] ❌ Button not found!")
		return
	end
	...
	if Necrosis.IsSpellKnown("soulstone") then
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] ✅ Soulstone spell known: " .. tostring(str))
	else
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] ⚠️ Soulstone spell NOT learned yet, configuring default buttons")
	end
	...
	if NecrosisConfig.ItemSwitchCombat[4] then
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] Left click: Using item " .. tostring(NecrosisConfig.ItemSwitchCombat[4]))
		f:SetAttribute("type1", "item")
		f:SetAttribute("item1", NecrosisConfig.ItemSwitchCombat[4])
	else
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] Left click: Casting spell")
		f:SetAttribute("type1", "spell")
		f:SetAttribute("spell1", str)
	end

	-- Right click to create new soulstone
	_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] Right click: Creating new soulstone")
	...
	if NecrosisConfig.ItemSwitchCombat[4] then
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] Middle click: Using item macro")
		...
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[SetSSAttribs] Middle click: Casting spell macro")
		f:SetAttribute("macrotext3", "/stopcasting \n/cast "..str)
	end
...
function Necrosis:MainButtonAttribute(self)
	...
	-- Skip if this is the placeholder frame (not a real button yet)
	if f:GetObjectType() ~= "Button" then
		print("MainButtonAttribute: Skipping placeholder, real button not created yet")
		return
	end
...
function Necrosis:HealthstoneUpdateAttribute(nostone)
	...
	if InCombatLockdown() or not f then
		_G["DEFAULT_CHAT_FRAME"]:AddMessage("[HealthstoneUpdateAttribute] ⚠️ In combat or button not found")
		return
	end
	...
	if NecrosisConfig.ItemSwitchCombat[3] then
		-- Left click: use the healthstone item
		f:SetAttribute("type1", "macro")
		f:SetAttribute("macrotext1", "/stopcasting \n/use "..NecrosisConfig.ItemSwitchCombat[3])
		-- Right click: create a new healthstone
		f:SetAttribute("type2", "spell")
...
function Necrosis:SpellstoneUpdateAttribute(nostone)
 ...
 --Necrosis.Debug.buttons = true
	...
	if nostone then
		f:SetAttribute("type1", "spell")
		f:SetAttribute("spell1", GetStoneSpellName("spellstone"))
		f:SetAttribute("type2", "spell")
		f:SetAttribute("spell2", GetStoneSpellName("spellstone"))
		return
	end
...
function Necrosis:FirestoneUpdateAttribute(nostone)
	...
	if nostone then
		f:SetAttribute("type1", "spell")
		f:SetAttribute("spell1", GetStoneSpellName("firestone"))
		f:SetAttribute("type2", "spell")
		f:SetAttribute("spell2", GetStoneSpellName("firestone"))
		return
	end
